...
function registerEventRoute(fastify: FastifyInstance){
    fastify.post<{Body: EventCreate}>('/', async(req, reply)=> {
        const { title,description,capacity,location,categoryId,startDate,endDate,format,producerId,ageRating,additionalDetails,creatorId} = req.body;
    try {
        const data = await eventUseCase.create({ title,description,capacity,location,categoryId,startDate,endDate,format,producerId,ageRating,additionalDetails,creatorId});
